...
class Module(LightningModule):
    ...
    def _criterion(self, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """Compute the (optionally label-smoothed) cross-entropy loss.
    ...
    def validation_step(self, data: Data) -> None:
        kwargs = self._prepare_forward_kwargs(data)
        preds: Tensor = self(data.x, data.edge_index, data.edge_attr, **kwargs)
        # The validation loss uses the same (optionally label-smoothed) objective as training so
        # that the monitored val/loss is consistent with the training objective used for early
